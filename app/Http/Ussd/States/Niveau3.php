...
class Niveau3 extends State
{
    protected function beforeRendering(): void
    {
		...
		// Récupération des sous-menus de niveau 3
	...
	/*protected function afterRendering(string $argument): void
    {
        $this->decision->custom2(
            ...
            function ($argument) {
                return UssdHelpers::handleCheckResult($this->record, $argument, 'checkIn', Sousmenu3::class);
            },
            function ($argument) {
                return UssdHelpers::handleCheckResult($this->record, $argument, 'checkInSoumenuThree', Sousmenu3::class);
            },
            Niveau4::class, List2::class, EndOfMenu::class
        )
            ->custom(
                function ($argument) {
                    $choix = false;
        ...
        $this->decision->custom2(
            function ($argument) {
                ...
                foreach($listes as $p){
                    // dd(gettype($p->selecteur));
                    if($p['key'] == $argument) {
                        $checkResult = checkInSoumenuThree($p['id']);

                        if($checkResult['isEnd']  == true) {
                            $this->record->set('fin', $checkResult['keyword']);
                            $choix=true;
                        }
                ...
                foreach($listes as $p){
                    // dd(gettype($p->selecteur));
                    if($p['key'] == $argument) {
                        $checkResult = checkIn(Sousmenu3::class, $p['id']);
